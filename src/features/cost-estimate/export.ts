...
function drawHeader(
  ...
  if (logo) {
    ...
    const logoLeftX = 36;
    const fittedLogo = fitImageWithinBox(logo, logoMaxWidth, logoBoxHeight);
    const logoX = logoLeftX;
    const logoY = logoBoxY + (logoBoxHeight - fittedLogo.height) / 2;
  ...
  doc.setTextColor(...BRAND.offWhite);
  doc.setFontSize(17);
  doc.text('COST ESTIMATE', pageWidth / 2, 70, { align: 'center' });

  setFont(doc);
  doc.setFontSize(8);
  doc.text('INTERIOR DESIGN & EXECUTION', pageWidth / 2, 88, {
    align: 'center',
  });
...
function drawSummaryAndTerms(
  ...
  const pageWidth = doc.internal.pageSize.getWidth();
  const y = ensureSpace(doc, 250, startY);
  const summaryX = pageWidth - 262;
  const summaryWidth = 226;
  const termsX = 36;
  const termsWidth = pageWidth - summaryWidth - 88;
  ...
  doc.setFontSize(10);
  doc.setTextColor(...BRAND.pearlBlack);
  doc.text('Terms & Conditions', termsX, y + 12);
  ...
  terms.forEach((term, index) => {
    const wrappedText = doc.splitTextToSize(`${index + 1}. ${term}`, termsWidth);

    doc.text(wrappedText, termsX, termsY);
    termsY += wrappedText.length * 10 + 6;
  });
}
...
export async function exportCostEstimatePdf(payload: CostEstimateExportPayload) {
  ...
  drawSummaryAndTerms(doc, payload, terms, getLastAutoTableFinalY(doc) + 24);
  drawFooter(doc);
